...
class TemporalTagger:
    def __init__(self):
        self.contradiction_markers = [
            'actually', 'changed', 'no longer', 'moved to', 'switched to'
        ]
        
    def _generate_fact_id(self, content: str, session_id: str) -> str:
        return hashlib.sha256(f"{content}:{session_id}".encode('utf-8')).hexdigest()
        
    def detect_supersessions(self, facts_by_entity: Dict[str, List[tuple[ExtractedFact, str]]]) -> List[TemporalRelation]:
        """
        ...
        for entity_name, facts in facts_by_entity.items():
            for i in range(len(facts)):
                for j in range(i + 1, len(facts)):
                    ...
                    similarity = fuzz.partial_ratio(old_fact.content.lower(), new_fact.content.lower())
